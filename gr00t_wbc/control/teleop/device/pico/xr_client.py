...
class XrClient:
    ...
    # --- CHANGED: Return INT (1/0) for state check ---
    def get_hand_tracking_state(self, hand: str) -> int:
        """Returns 1 if hand tracking is active, 0 otherwise."""
        if hand.lower() == "left":
            return 1 if xrt.get_left_hand_is_active() else 0
        elif hand.lower() == "right":
            return 1 if xrt.get_right_hand_is_active() else 0
        else:
            raise ValueError(f"Invalid hand: {hand}. Valid hands are: 'left', 'right'.")

    # --- ADDED: Return actual joint data ---
    def get_hand_joint_locations(self, hand: str) -> np.ndarray | None:
        """Returns the 27x7 joint array [x,y,z,qx,qy,qz,qw] directly."""
        # DEBUG: FORCE READ - Comment out the safety check
        # if hand.lower() == "left":
        ...
        try:
            ...
            print(f"Error forcing hand read: {e}")
            return None
