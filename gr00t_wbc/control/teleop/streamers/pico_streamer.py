...
class PicoStreamer(BaseStreamer):
    ...
    def _get_pico_data(self):
        ...
        pico_data["right_menu_button"] = self.xr_client.get_button_state_by_name("right_menu_button")
        ...
        # --- NEW: Get Hand Joint Locations ---
        # We use getattr/try-except to be safe if method doesn't exist yet
        try:
            # Assuming get_hand_joint_locations exists in your XrClient
            # If the method is named differently (e.g. get_joints), change it here.
            pico_data["left_hand_joints"] = self.xr_client.get_hand_joint_locations("left")
            pico_data["right_hand_joints"] = self.xr_client.get_hand_joint_locations("right")
        except AttributeError:
            pico_data["left_hand_joints"] = None
            pico_data["right_hand_joints"] = None
        # -------------------------------------
    ...
    def _generate_unified_raw_data(self, pico_data):
        ...
        # Get gripper/hand commands
        # Pass headset pose to correctly orient hand skeleton
        left_fingers = self._generate_finger_data(pico_data, "left", pico_data["head_pose"])
        right_fingers = self._generate_finger_data(pico_data, "right", pico_data["head_pose"])
    ...
    def _generate_finger_data(self, pico_data, hand, headset_pose): 
        """
        Generate finger position data.
        Trusts joint data availability over the 'tracking_state' flag.
        """
        # 1. Retrieve data
        tracking_state = pico_data.get(f"{hand}_hand_tracking_state")
        hand_joints = pico_data.get(f"{hand}_hand_joints")


        # 2. Check if valid joints exist (length >= 25)
        hand_tracking_active = (hand_joints is not None and len(hand_joints) >= 25)
