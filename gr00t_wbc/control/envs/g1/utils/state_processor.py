...
from gr00t_wbc.control.envs.g1.utils.inspire_driver import InspireHandDriver
...
class InspireHandStateProcessor:
    def __init__(self, is_left: bool = True):
        ...
        # Inspire has 6 active DOFs.
        # But Groot pipeline usually expects 7 for Dex3 compatibility in arrays.
        # We will pad the 7th value with 0 to prevent shape mismatch errors downstream.
        self.active_dof = 6
        self.pad_dof = 7 
    ...
    def _prepare_low_state(self) -> np.ndarray:
        ...
        # Helper to pad array from 6 to 7
        def pad(arr):
        ...
        dq_pad = pad(dq)
        tau_pad = pad(tau)
        ddq_pad = np.zeros(self.pad_dof) # No acceleration data
